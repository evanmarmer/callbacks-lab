...
const startsWith = (word) => word[0] === 'a' || word[0] === 'A';

function startWithLetterA(arr){
  const newArr = arr.filter(startsWith)
  return newArr
}
...
function findProgrammer(empList){
  let newArr = empList.filter((element,index,array) => {
...
function productOfArray(array){
  let product = array.reduce((acc, cur, index, array) => {
    return acc * cur
  })
  return product
}
